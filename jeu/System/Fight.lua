...
function Combats(joueur, monstre)
    ...
    if choix == 1 then
        ...
        local dmg_joueur = Calcul_dmg_j(joueur,monstre)
        monstre:loseHp(dmg_joueur)

        if monstre:getHp() < 0 then
            monstre:setHp(0)
        end

        love.window.showMessageBox("Attaque", "Vous avez infligé " .. dmg_joueur .. " dégâts au monstre.\nIl lui reste " .. monstre:getHp() .. " HP.", {"OK"})
        ...
        if winorlose == nil then 
            local dmg_monstre = Calcul_dmg_m(monstre,joueur)
            joueur:loseHp(dmg_monstre)

            if joueur:getHp() < 0 then
                joueur:setHp(0)
            end

            love.window.showMessageBox("Attaque", "Le monstre vous a infligé " .. dmg_monstre .. " dégâts.\nIl vous reste " .. joueur:getHp() .. " HP.", {"OK"})
    ...
    elseif choix == 2 then
        ...
        if cd > 0 then
            ...
            if classe == "Archer" then
                ...
                dmg_joueur = dmg_joueur + Calcul_dmg_j(joueur,monstre)
                monstre:loseHp(dmg_joueur)
                ...
                love.window.showMessageBox("Compétence", "Vous avez infligé " .. dmg_joueur .. " dégâts au monstre.\nIl lui reste " .. monstre:getHp() .. " HP.", {"OK"})
                winorlose = WinOrLose(joueur,monstre)
